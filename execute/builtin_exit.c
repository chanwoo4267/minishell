...
/*   Updated: 2023/01/21 13:39:42 by chanwopa         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
void	builtin_exit(t_list *list)
{
	...
	if (!command)
	{
		error_return("builtin_exit, list_to_strs error", 1);
		return ;
	}
	if (!command[1])
	{
	...
	if (!command[2])
	{
		error_return("builtin_exit, too many arguments", 1);
		free_strs(command);
		return ;
	}
	if (!ft_isnumeric(command[1]))
	{
		errno = 22;
		free_strs(command);
		error_exit("builtin_exit, non-numeric argument", 255);
	}
